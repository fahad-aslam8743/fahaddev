...
export async function POST(req:Request){
  ...
  const phone=String(body.phone||'').trim().replace(/[^0-9+()\-\s]/g,'').slice(0,30);
  ...
  if(name.length<2||!emailPattern.test(email)||phone.replace(/\D/g,'').length<7||message.length<20)return NextResponse.json({error:'Please add your name, a valid email, WhatsApp/phone number and a little more project detail.'},{status:400});
  const {data,error}=await sb.from('leads').insert({name,email,phone,company:company||null,website_url:websiteUrl||null,project,project_size:projectSize||null,message,status:'new',source}).select().single();
