...
export default async function sitemap():Promise<MetadataRoute.Sitemap>{
  ...
  return [...base.map(x=>({url:new URL(x.path,SITE_URL).toString(),changeFrequency:x.changeFrequency,priority:x.priority})),...services.map(s=>({url:new URL(`/services/${s.slug}`,SITE_URL).toString(),changeFrequency:'monthly' as const,priority:.82})),...projects.map(p=>({url:new URL(`/work/${p.slug}`,SITE_URL).toString(),changeFrequency:'monthly' as const,priority:.72}))];
}
