...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{const {slug}=await params;const p=await getProject(slug);if(!p)return {};return pageMetadata({title:`${p.title} Case Study — ${p.clientType}`,description:p.short,path:`/work/${p.slug}`,image:p.imageUrl||'/heroes/work.png'})}
export default async function CaseStudy({params}:{params:Promise<{slug:string}>}){
  ...
  return <article className="case shell"><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema).replace(/</g,'\\u003c')}}/><nav className="breadcrumb" aria-label="Breadcrumb"><Link href="/">Home</Link><span>/</span><Link href="/work">Work</Link><span>/</span><span>{p.title}</span></nav><div className="case-head">{p.concept&&<span className="concept">Concept</span>}<h1>{p.title}</h1><p>{p.short}</p><div className="case-meta"><span>{p.clientType}</span><span>{p.timeline}</span><span>{p.stack.join(' · ')}</span></div></div>
  {p.imageUrl?<figure className="case-project-image"><Image src={p.imageUrl} alt={`${p.title} project screenshot`} fill priority sizes="(max-width: 1180px) 100vw, 1180px"/></figure>:<div className={`case-visual visual-${p.slug}`}><div className="browser"><span/><span/><span/></div><div className="mock-copy"><small>{p.clientType}</small><strong>{p.title}</strong><em>{p.concept?'Concept product study':'Product system case study'}</em></div></div>}
