    ...
    <section className="engine-hero-section engine-case-hero">
      ...
      <div className="shell engine-hero-content">
        <div className="engine-hero-copy case-head">
          ...
          {p.concept&&<span className="concept">Concept study</span>}
          <span className="eyebrow">{p.clientType}</span>
          <h1>{p.title}</h1>
          <p>{p.short}</p>
          <div className="case-meta"><span>{p.timeline}</span><span>{p.stack.join(' · ')}</span></div>
          {p.liveUrl&&<a className="btn" href={p.liveUrl} target="_blank" rel="noreferrer">Visit live site <ArrowUpRight size={17}/></a>}
        </div>
      </div>
    </section>
