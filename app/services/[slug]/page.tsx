...
export default async function ServiceDetail({params}:{params:Promise<{slug:string}>}){
  ...
  return <>
    ...
    <section className="service-detail-hero"><div className="shell"><nav className="breadcrumb" aria-label="Breadcrumb"><Link href="/">Home</Link><span>/</span><Link href="/services">Services</Link><span>/</span><span>{service.shortTitle}</span></nav><div className="service-detail-head"><div className="service-detail-copy"><span className="eyebrow">{service.eyebrow}</span><h1>{service.title}</h1><p>{service.description}</p><div className="hero-actions"><Link className="btn" href="/contact">Discuss this project <ArrowRight size={17}/></Link><Link className="btn btn-secondary" href="/work">See related work</Link></div><p className="microproof"><CheckCircle2 size={15}/> No technical brief needed <span>·</span> Scope before commitment <span>·</span> Remote-friendly</p><div className="service-best-fit"><Icon/><div><span>Best fit</span><p>{service.bestFor}</p></div></div></div><div className="service-detail-visual"><HeroSystem kind="services" context={service.shortTitle}/></div></div></div></section>
