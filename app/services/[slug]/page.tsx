...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{const {slug}=await params;const service=getService(slug);if(!service)return {};return pageMetadata({title:service.title,description:service.description,path:`/services/${service.slug}`,image:'/heroes/services.png'})}
...
export default async function ServiceDetail({params}:{params:Promise<{slug:string}>}){
  ...
  return <>
    ...
    <section className="service-detail-hero"><div className="shell"><nav className="breadcrumb" aria-label="Breadcrumb"><Link href="/">Home</Link><span>/</span><Link href="/services">Services</Link><span>/</span><span>{service.shortTitle}</span></nav><div className="service-detail-head"><div><span className="eyebrow">{service.eyebrow}</span><h1>{service.title}</h1><p>{service.description}</p><div className="hero-actions"><Link className="btn" href="/contact#project-brief">Discuss this project <ArrowRight size={17}/></Link><Link className="btn btn-secondary" href="/work">See related work</Link></div><p className="microproof"><CheckCircle2 size={15}/> No technical brief needed <span>·</span> Scope before commitment <span>·</span> Remote-friendly</p></div><div className="service-detail-card"><Icon/><span>Best fit</span><p>{service.bestFor}</p></div></div></div></section>
