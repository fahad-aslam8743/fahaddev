...
export function LeadForm(){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    const payload={
      name:fd.get('name'), email:fd.get('email'), company:fd.get('company'), website_url:fd.get('website_url'),
  ...
  return <form className="contact-form" onSubmit={submit}>
    <div className="contact-form-heading"><span className="eyebrow">Project brief</span><h3>Give me enough context to make the first reply useful.</h3><p>No technical language required. No commitment is created by sending this form.</p></div>
    ...
    <div className="form-two"><label>Company / project<input name="company" maxLength={120} placeholder="Optional"/></label><label>Existing website<input name="website_url" type="url" maxLength={220} placeholder="https://... (optional)"/></label></div>
    ...
    <p className="form-note">Your details are used to respond to this project enquiry. <Link href="/privacy">Privacy details</Link> · Prefer a faster conversation? WhatsApp is available above.</p>
  </form>;
}
